    ...
    <div class="card">
        <div class="card-header"
            ...
            <div style="display: flex; gap: 10px;">
                <button class="btn btn-success" onclick="showModal('importProductsModal')"
                    style="background: #27ae60; border-color: #27ae60; color: #fff;">
                ...
                <tbody>
                    <?php foreach ($products as $p): ?>
                        <tr>
                            <td>
                                <?php if ($p->image): ?>
                                    <img src="<?php echo base_url('assets/images/products/' . $p->image); ?>" alt=""
                            ...
                            <td>
                                ...
                                if (in_array('1', $p_shops))
                                    echo '<span class="badge" style="background:#3498db; color:#fff; padding:4px 8px; border-radius:10px; font-size:0.7rem; margin-right:4px; margin-bottom:2px; display:inline-block; white-space:nowrap; vertical-align:middle;">Villiers-le-bel</span>';
                                if (in_array('2', $p_shops))
                                    echo '<span class="badge" style="background:#9b59b6; color:#fff; padding:4px 8px; border-radius:10px; font-size:0.7rem; margin-bottom:2px; display:inline-block; white-space:nowrap; vertical-align:middle;">Le Plessis-Bouchard</span>';
                                ?>
                            </td>
                            <td>
                                <span class="badge"
                                    style="background: var(--success); color: #fff; padding: 5px 10px; border-radius: 15px; font-size: 0.75rem;">Active</span>
                            </td>
                            <td>
                                <a href="<?php echo base_url('admin/edit_product/' . $p->id); ?>"
                                    class="btn btn-primary btn-sm" style="background: var(--info);">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" class="btn btn-danger btn-sm"
                                    onclick="showConfirm('<?php echo base_url('admin/delete_product/' . $p->id); ?>')">
    ...
    <div class="modal-content">
        ...
        <div class="modal-body" style="max-height: 80vh; overflow-y: auto;">
            ...
            <div class="form-group"
                style="margin-bottom: 20px; background: #f8fafc; padding: 15px; border-radius: 10px; border: 1px solid #e2e8f0;">
                <label style="display:block; margin-bottom: 8px; color: #1e293b;"><strong>Available Shop
                        Location(s)</strong></label>
                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                    <label
                        style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 600; background: #fff; padding: 8px 14px; border-radius: 8px; border: 1px solid #ddd; border-left: 4px solid #e74c3c;">
                        <input type="checkbox" name="shops[]" value="1" checked
                            style="width: 18px; height: 18px; accent-color: #e74c3c;">
                        <span style="color: #c0392b;">Villiers-le-bel</span>
                    </label>
                    <label
                        style="display: flex; align-items: center; gap: 8px; cursor: pointer; font-weight: 600; background: #fff; padding: 8px 14px; border-radius: 8px; border: 1px solid #ddd; border-left: 4px solid #2563eb;">
                        <input type="checkbox" name="shops[]" value="2" checked
                            style="width: 18px; height: 18px; accent-color: #2563eb;">
                        <span style="color: #2563eb;">Le Plessis-Bouchard</span>
                    </label>
                </div>
                <small style="color: #7f8c8d; margin-top: 5px; display: block;"><i class="fas fa-info-circle"></i>
                    Choose which shop(s) offer this product.</small>
            </div>
            <!-- Step 1: Select Category -->
            <div class="form-group">
                <label><strong>1. Category</strong></label>
                <select name="category_id" id="categorySelect" class="form-control" required
                    onchange="loadSubCategoriesAndSizes(this.value)">
            ...
            <div class="form-group">
                <label><strong>Select Offer</strong> <span
                        style="color: var(--gray); font-weight: 400;">(Optional)</span></label>
                <select name="offer_id" class="form-control">
                    <option value="">None</option>
                    <?php foreach ($offers as $offer): ?>
                        <option value="<?php echo $offer->id; ?>"><?php echo htmlspecialchars($offer->offer_name); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <div id="addonGroupsPanel"
                    style="border: 1px solid #ddd; border-radius: 10px; overflow: hidden; max-height: 320px; overflow-y: auto;">
                        ...
                        <?php foreach ($addon_groups as $gi => $group): ?>
                            <div class="ag-block" style="border-bottom: 1px solid #eee;">
                                ...
                                " onclick="toggleGroup(<?php echo $gi; ?>)">
                                    <!-- Group checkbox -->
                                    <input type="checkbox" name="product_addon_group_ids[]" value="<?php echo $group->id; ?>"
                                    ...
                                    ?>
                                    <span
                                        style="font-size:0.7rem;background:#ede7f6;color:#7b1fa2;padding:2px 8px;border-radius:20px;font-weight:600;">
                                        <?php echo $cond; ?>
                                    </span>
                                    <!-- Addon count badge -->
                                    <span
                                        style="font-size:0.72rem;background:#eee;color:#888;padding:2px 8px;border-radius:20px;">
                                        <?php echo count($group->addons ?? []); ?>
                                        addon<?php echo count($group->addons ?? []) != 1 ? 's' : ''; ?>
                                    </span>
                                    <!-- Toggle arrow -->
                                ...
                                <div class="ag-addons" id="ag_<?php echo $gi; ?>"
                                    style="display:none; background:#fff; padding:8px 14px 12px 36px;">
                                    <?php if (!empty($group->addons)): ?>
                                        <div
                                            style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:7px;margin-top:6px;">
                                            <?php foreach ($group->addons as $addon): ?>
                                                <label
                                                    ...
                                                    <span>
                                                        <strong
                                                            style="color:#2c3e50;"><?php echo htmlspecialchars($addon->name); ?></strong>
                    ...
                    <?php else: ?>
                        <div style="padding:20px;text-align:center;color:#ccc;">
                            <i class="fas fa-layer-group"
                                style="font-size:2rem;display:block;margin-bottom:8px;opacity:0.3;"></i>
                ...
                <small style="color:#999;margin-top:6px;display:block;">
                    ...
                    Check a <strong>group</strong> to link it to the product. Check individual <strong>addons</strong>
            ...
            <div class="form-group">
                <label><strong>5. Base Price (€)</strong> <span
                        style="color: var(--gray); font-weight: 400;">(Optional)</span></label>
...
<div id="importProductsModal" class="custom-modal">
    <div class="modal-content" style="max-width: 650px;">
        ...
        <div class="modal-body" style="max-height: 80vh; overflow-y: auto; padding: 20px;">
            ...
            <div
                style="background: #eef9f1; border: 1px solid #c8e6c9; padding: 15px; border-radius: 10px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
                <div>
                    <strong style="color: #1e7e34; font-size: 0.95rem;"><i class="fas fa-download"></i> Download CSV
                        Template</strong>
                </div><br>
                <a href="<?php echo base_url('admin/download_product_sample_csv'); ?>" class="btn btn-sm"
                    style="background: #27ae60; color: #fff; text-decoration: none; padding: 8px 14px; border-radius: 6px; font-weight: 600; font-size: 0.85rem;">
            ...
            <div class="form-group" style="margin-bottom: 20px;">
                ...
                <input type="file" name="csv_file" class="form-control" accept=".csv, text/csv, application/csv"
                    required
                    style="padding: 10px; border: 2px dashed #3498db; background: #f8fafc; border-radius: 8px; cursor: pointer;">
            </div>


            <button type="submit" class="btn btn-success"
                style="width: 100%; padding: 12px; font-weight: bold; background: #27ae60; border-color: #27ae60; font-size: 1rem;">
